    ...
    for (auto i = 0; i < getLayers(); i++)
    {
        if (i + 1 < getLayers())
        {
            ...
            case Softshrink:
                ...
                break;
            case Sine:
                x = torch::sin(conv[i](x));
                break;
            case Sine30:
                x = torch::sin(30 * conv[i](x));
                break;
            default:
                x = (conv[i](x));
                break;
...
void Model::initModel(int seed)
{
    ...
    for (auto i = 0; i < getLayers(); i++)
    {
        ...
        case uniform1:
            torch::nn::init::uniform_(conv[i]->weight, -0.25, 0.25);
        case uniform2:
            torch::nn::init::uniform_(conv[i]->weight, -1.00, 1.00);
        case xavier_normal:
